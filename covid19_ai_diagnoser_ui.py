...
screenWidth = "1560"
screenHeight = "840"
windowTitle = "Smart Covid 19 Diagnosis Tool"
...
class Window(Frame):
    ...
    # Added to facilitate output window data
    ...
    def __init__(self, master=None):
        ...
        img.image = render
        img.place(
            x=(int(screenWidth) / 2) - load.width / 2,
            y=((int(screenHeight) / 2)) - load.height / 2 - 80,
        )
    ...
    def addDiagnosisResult(self, value):
        self.DIAGNOSIS_RESULT_FIELD.delete(
            "1.0", "end"
        )  # clear diagnostic result text element
...
def loadRegularPneumoniaImageFromDialog():
    ...
    image_file = filedialog.askopenfile(
        ...
        parent=root,
        initialdir=currdir,
        title="Please select an Xray Image of suspected regular pneumonia case:",
    )
...
def loadRegularPneumoniaImageFromName(filename):
    ...
    load = load.resize(
        (CONSTANT_DIAGNOSIS_IMAGE_SPAN, CONSTANT_DIAGNOSIS_IMAGE_SPAN), Image.ANTIALIAS
    )  # Resized "load" image to constant size on screen. However, neural network still runs on on original image scale from filename.
    render = ImageTk.PhotoImage(load)
    img = Label(image=render)
    img.image = render
    img.place(
        x=(int(screenWidth) / 2) - CONSTANT_DIAGNOSIS_IMAGE_SPAN / 2,
        y=((int(screenHeight) / 2)) - CONSTANT_DIAGNOSIS_IMAGE_SPAN / 2 - 80,
    )
    app.DIAGNOSIS_RESULT += "**Non-Covid19 Mode Result**\n" + filename + "\n\n"
    app.DIAGNOSIS_RESULT += covid19_ai_diagnoser.doOnlineInference_regularPneumonia(
        filename
    )
...
def loadCovid19ImageFromName(filename):
    ...
    app.DIAGNOSIS_RESULT += covid19_ai_diagnoser.doOnlineInference_covid19Pneumonia(
        filename
    )
    print(app.DIAGNOSIS_RESULT)
    app._PRIOR_IMAGE = img  # set latest instance of old image
...
filemenu.add_command(
    label="Load image to test for pneumonia",
    command=loadRegularPneumoniaImageFromDialog,
)
filemenu.add_command(
    label="Load image to test for covid-19", command=loadCovid19ImageFromDialog
)
...
def colourDiagnosisMessageText(diagnosisContent, startIndexText, endIndexText):
    ...
    if (
        covid19_ai_diagnoser.DIAGNOSIS_MESSAGES[0] in diagnosisContent
        or covid19_ai_diagnoser.DIAGNOSIS_MESSAGES[1] in diagnosisContent
    ):
        app.DIAGNOSIS_RESULT_FIELD.tag_add(
            "DIAGNOSIS_RESULT_MESSAGE", startIndexText, endIndexText
        )
        app.DIAGNOSIS_RESULT_FIELD.tag_configure(
            "DIAGNOSIS_RESULT_MESSAGE", background="red", foreground="white"
        )
    ...
    if covid19_ai_diagnoser.DIAGNOSIS_MESSAGES[2] in diagnosisContent:
        ...
        app.DIAGNOSIS_RESULT_FIELD.tag_configure(
            "DIAGNOSIS_RESULT_MESSAGE", background="green", foreground="white"
        )
