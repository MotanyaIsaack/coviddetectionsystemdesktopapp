...
model_pneumoniaDetector = (
    covid19_ai_diagnoser_optimal_model_architecture.model_pneumoniaDetector
)
model_covid19PneumoniaDetector = (
    covid19_ai_diagnoser_optimal_model_architecture.model_covid19PneumoniaDetector
)
...
###########################
# Function to do online inference i.e. Regular Pneumonia tests
def doOnlineInference_regularPneumonia(imagePath):
    test_data = []
    img = covid19_ai_diagnoser_optimal_model_architecture.cv2.imread(
        imagePath, 0
    )  # Replace plt.imread, with  gray scale cv2.imread(path,0), so that ui's image load process doesn't throw a pyimage10 error
    img = covid19_ai_diagnoser_optimal_model_architecture.cv2.resize(
        img,
        (
            covid19_ai_diagnoser_optimal_model_architecture.img_dims,
            covid19_ai_diagnoser_optimal_model_architecture.img_dims,
        ),
    )
    ...
    prediction = model_pneumoniaDetector.predict(
        covid19_ai_diagnoser_optimal_model_architecture.np.array(test_data)
    )
    ...
    outputContent += (
        ...
        + str(prediction[0][0])
        + ". A value closer to 1 signifies illness, while a value closer to 0 signifies normalness.\n\n"
    )
    recordInferenceEvent(imagePath, outputContent)
    return outputContent


# Function to do online inference i.e. Covid19 tests
def doOnlineInference_covid19Pneumonia(imagePath):
    ...
    print(img.shape)
    test_data.append(img)
    model_covid19PneumoniaDetector.save("covid_final_model.h5")
    covid_final_model = load_model("to_use_final_model.h5")

    prediction = covid_final_model.predict(np.array(test_data))
    _prediction = round(prediction[0][0] * 100, 3)
    if _prediction > 50:
