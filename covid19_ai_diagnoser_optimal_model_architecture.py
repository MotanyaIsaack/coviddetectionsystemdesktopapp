...
def renderConfusionMetrics(
    ...
    __batch_size,
    __epochs,
    hdf5_testSaveFileName,
):
    ___model.save("original_final_model.h5")
    ...
    print("Recall/Sensitivity: {}%".format(recall))  # added sensitivity label
    print("Specificity {}%".format(specificity))  # added specificity calculation
    print("F1-score: {}".format(2 * precision * recall / (precision + recall)))

    if enableTraining:
        checkpoint = ModelCheckpoint(
            filepath=hdf5_testSaveFileName, save_best_only=True, save_weights_only=True
        )
        lr_reduce = ReduceLROnPlateau(
            monitor="val_loss", factor=0.3, patience=2, verbose=2, mode="max"
        )
        early_stop = EarlyStopping(
            monitor="val_loss", min_delta=0.1, patience=1, mode="min"
        )

        hist = ___model.fit_generator(
            ___train_gen,
            steps_per_epoch=___test_gen.samples // __batch_size,
            epochs=__epochs,
            validation_data=___test_gen,
            validation_steps=___test_gen.samples // __batch_size,
            callbacks=[checkpoint, lr_reduce],
        )
        ___model.save("final_model.h5")
        print(hist.history)
        print("\nTRAIN METRIC ----------------------")
        print(
            "Covid19 Train acc: {}".format(
...
def process_data(___inputPath, img_dims, batch_size):
    ...
    train_datagen = ImageDataGenerator(
        rescale=1.0 / 255, zoom_range=0.3, vertical_flip=True
    )
    ...
    for cond in ["/NORMAL/", "/PNEUMONIA/"]:
        for img in os.listdir(___inputPath + "test" + cond):
            img = cv2.imread(
                ___inputPath + "test" + cond + img, 0
            )  # Replace plt.imread, with  gray scale cv2.imread(path,0), so that ui's image load process doesn't throw a pyimage10 error
...
def reportFileDistributions(___inputPath, directoryProcessArray):
    for _set in directoryProcessArray:
        ...
        print(
            "Set: {}, normal images: {}, illness-positive images: {}".format(
...
logging.getLogger(
    "tensorflow"
).disabled = True  # Disable red warning lines seen at model architecture creation.
...
model_pneumoniaDetector.compile(
...
# Getting the data
train_gen, test_gen, test_data_b, test_labels_b = process_data(
    input_path_b, img_dims, batch_size
)

# Reporting on accuracies
renderConfusionMetrics(
    model_pneumoniaDetector,
    test_data_b,
    test_labels_b,
    False,
    None,
...
model_covid19PneumoniaDetector.compile(
    optimizer="adam", loss="binary_crossentropy", metrics=["accuracy"]
)
model_covid19PneumoniaDetector.load_weights("covid19_neural_network_weights.hdf5")
model_covid19PneumoniaDetector.save("to_use_final_model.h5")
...
# establish custom_path for covid 19 test data
...
# Getting the data
train_gen_d, test_gen_d, test_data_d, test_labels_d = process_data(
    input_path_d, img_dims, batch_size
)

# Reporting on accuracies
renderConfusionMetrics(
    model_covid19PneumoniaDetector,
    test_data_d,
    test_labels_d,
    False,
    train_gen_d,
    ...
    batch_size,
    11,
    "covid19_neural_network_weights_v2.hdf5",
)
